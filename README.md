<p align="center">
  <a href="#한국어"><img src="https://img.shields.io/badge/%ED%95%9C%EA%B5%AD%EC%96%B4-0a0a0a?style=for-the-badge&labelColor=B8F564&color=ffffff" alt="한국어" /></a>
  <a href="#english"><img src="https://img.shields.io/badge/English-0a0a0a?style=for-the-badge&labelColor=0EA5E9&color=0a0a0a" alt="English" /></a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/%F0%9F%9F%A2%20Available%20for%20Opportunities-B8F564?style=for-the-badge&labelColor=0a0a0a&color=B8F564" alt="Available for Opportunities" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/%F0%9F%93%8D%20%20-South%20Korea-0EA5E9?style=for-the-badge&labelColor=0a0a0a" alt="South Korea" />
</p>

## 한국어

> Business Automation Specialist — 비즈니스 맥락과 데이터를 연결해, 실제 운영에 쓰이는 AI 자동화를 만듭니다.

### 👋 About Me

~~~json
about = {
  "name":       "정안용",
  "role":       "운용 자동화 전문가",
  "focus":      ["AI" "데이터", "자동화", "SEO/GEO", "데이터분석"],
  "philosophy": "다양한 경험을 거름으로 삼아 현장의 목소리를 듣는 개발자",
  "location":   "대한민국"
}
~~~

저는 **흩어진 정보**를 명확한 의사결정으로, **반복 수작업**을 문서화된 운영 시스템으로 바꿉니다. 제조·마케팅 현장 경험을 바탕으로 Python/JavaScript, LLM 워크플로, SEO/GEO, 데이터 분석을 함께 다룹니다.

### 🛠️ Tech Stack

#### Data & BI

![Excel](https://img.shields.io/badge/EXCEL-217346?style=for-the-badge&logo=microsoftexcel&logoColor=ffffff) ![SQL Server](https://img.shields.io/badge/SQL%20SERVER-D32F2F?style=for-the-badge&logo=microsoftsqlserver&logoColor=ffffff)

#### Programming

![Python](https://img.shields.io/badge/PYTHON-3776AB?style=for-the-badge&logo=python&logoColor=ffffff) ![JavaScript](https://img.shields.io/badge/JAVASCRIPT-F7DF1E?style=for-the-badge&logo=javascript&logoColor=111827)

#### AI & Automation

![OpenAI](https://img.shields.io/badge/OPENAI-4C1D95?style=for-the-badge&logo=openai&logoColor=ffffff) ![Claude](https://img.shields.io/badge/CLAUDE-D97757?style=for-the-badge&logo=anthropic&logoColor=ffffff) ![Antigravity](https://img.shields.io/badge/ANTIGRAVITY-7C3AED?style=for-the-badge&logoColor=ffffff)

### 🚀 Featured Projects

#### 📰 Factcheck & AI 뉴스 허브

인스타그램, X, Threads 등에서 바이럴 되는 게시글에 대하여, AI가 사실을 검증하는 리서치 포트폴리오입니다.

<p>
  <a href="https://annyeonghae.github.io/ai-factcheck-portfolio/"><img src="https://img.shields.io/badge/%E2%96%B6%20Live%20Demo-B8F564?style=for-the-badge&labelColor=0a0a0a" alt="Live Demo" /></a>
  <a href="https://github.com/AnnyeongHae/ai-factcheck-portfolio"><img src="https://img.shields.io/badge/GitHub%20Repo-181717?style=for-the-badge&logo=github" alt="Repo" /></a>
</p>

#### 🖼️ 이미지 RAG

공개된 이미지·프롬프트를 수집해 보여주고, RAG파이프라인을 통해 API 사용자에게 '관련 이미지'를 검색할 수 있도록 합니다.

<p>
  <a href="https://photoposting.shop"><img src="https://img.shields.io/badge/%E2%96%B6%20Live%20Demo-B8F564?style=for-the-badge&labelColor=0a0a0a" alt="Live Demo" /></a>
  <a href="https://github.com/AnnyeongHae/Image_archive"><img src="https://img.shields.io/badge/GitHub%20Repo-181717?style=for-the-badge&logo=github" alt="Repo" /></a>
</p>

#### 🧪 E.F.M — Marketing & Internal Process Automation

- **블로그 자동화**: 키워드 추출, 뉴스/자료 마이닝, 사실 확인, LLM 초안 작성, HTML 포맷팅과 결과물 저장을 연결한 Python 워크플로
- **스마트스토어 운영 자동화**: Powerlink 키워드·광고 데이터 수집, SERP 파싱·분류로 광고 효율 개선을 지원하고 주문 확인·지연 문자·배송 처리를 자동화
- **스마트스토어 상세페이지 자동화**: 상품 이미지와 특징을 입력하면 카피·레이아웃·HTML 상세페이지 산출물을 생성하는 로컬 Python 파이프라인

#### 🌎 Nammigo — SEO/GEO & Media Asset Automation

- 해킹을 당해 SEO 최적하 최하위를 기록하던 WordPress 웹사이트를 상위노출로 리드
- 네이버와 LLM 검색을 고려한 남미 여행 콘텐츠 최적화
- Python 기반 사진 분류로 여행 이미지를 장소·날짜 기준으로 정리
- 마케팅 팀 대상 프롬프트 엔지니어링·이미지 생성·워크플로 통합 교육

### 📚 Published

1. [소셜네트워크 분석 방법을 활용한 코로나 전, 후의 여행 트렌드 파악: <여행매거진>을 중심으로](https://www.dbpia.co.kr/journal/articleDetail?nodeId=NODE11300884) — 정안용·박진영·남윤재 (2021)
2. [네이버 뉴스 댓글을 활용한 ‘비혼출산’에 대한 감성분석](https://doi.org/10.5392/JKCA.2022.22.01.074) — 허세영·김초원·정안용·이새봄 (2022)
3. [텍스트마이닝과 네트워크분석을 활용한 스마트도시 언론보도 분석](https://doi.org/10.37727/jkdas.2022.24.4.1563) — 정안용·박진영·남윤재 (2022)
4. [쌍방향 대화원칙을 통한 한국과 미국의 음악저작권협회 홈페이지 비교분석 연구](https://doi.org/10.9728/dcs.2024.25.7.1931) — 석화윤·박태희·정안용·남윤재 (2024)
5. [VR, AR, 메타버스 관광의 종단적 분석: 다이나믹 토픽 모델링을 활용한 언론 매체의 토픽 변화 연구(2016–2023)](https://doi.org/10.17086/JTS.2025.49.5.31.54) — 윤성은·정안용·김예림·남윤재 (2025)
6. [한국 반려동물 양육자의 호텔 선택: 주요 속성](https://doi.org/10.1080/1528008X.2026.2639377) — 정안용·남윤재 (2026, *Pet Owners’ Hotel Choices in South Korea: Key Attributes*)
7. *Hotel Choice Attributes in Pet-friendly Hotel: Text Mining and Conjoint Analysis Approach* — 석사학위 연구

### 🏆 해커톤·수상

| 활동 | 결과·구분 | 기간 | 주관 | 기록 |
|---|---|---|---|---|
| 현업활용 우수사례 공모전 | 3등 · 공모전 수상 | 2025/11/28 | 한국관광공사 / 데이터마케팅코리아 | - |
| KB IT’s Your Life 해커톤 | 우수상 수상 · 경진대회/해커톤 | 2024/11/20 → 2024/11/22 | KB 국민은행 | [Notion](https://app.notion.com/154e4a6d03ba802295adc8e90ac3ef3f) |

---

## English

> Business Automation Specialist connecting business context, data, and practical AI workflows.

### 👋 About Me

~~~json
about = {
  "name":       "Cheong Anyong",
  "role":       "Business Automation Specialist",
  "focus":      ["Data & AI Operations", "Process Automation", "SEO/GEO", "Data Analytics"],
  "philosophy": "Turn scattered information and manual work into reviewable, reusable workflows.",
  "location":   "South Korea",
}
~~~

I turn **scattered information** into clear decisions and **repetitive manual work** into documented, repeatable systems. My work combines manufacturing and marketing context with Python/JavaScript, LLM workflows, SEO/GEO, and data analytics.

### 🛠️ Tech Stack

#### Data & BI

![Excel](https://img.shields.io/badge/EXCEL-217346?style=for-the-badge&logo=microsoftexcel&logoColor=ffffff) ![SQL Server](https://img.shields.io/badge/SQL%20SERVER-D32F2F?style=for-the-badge&logo=microsoftsqlserver&logoColor=ffffff)

#### Programming

![Python](https://img.shields.io/badge/PYTHON-3776AB?style=for-the-badge&logo=python&logoColor=ffffff) ![JavaScript](https://img.shields.io/badge/JAVASCRIPT-F7DF1E?style=for-the-badge&logo=javascript&logoColor=111827)

#### AI & Automation

![OpenAI](https://img.shields.io/badge/OPENAI-4C1D95?style=for-the-badge&logo=openai&logoColor=ffffff) ![Claude](https://img.shields.io/badge/CLAUDE-D97757?style=for-the-badge&logo=anthropic&logoColor=ffffff) ![Antigravity](https://img.shields.io/badge/ANTIGRAVITY-7C3AED?style=for-the-badge&logoColor=ffffff)

### 🚀 Featured Projects

#### 📰 Factcheck & AI News Hub

A public research portfolio connecting claims, sources, and review status so AI-assisted research can be checked by people.

<p>
  <a href="https://annyeonghae.github.io/ai-factcheck-portfolio/"><img src="https://img.shields.io/badge/%E2%96%B6%20Live%20Demo-B8F564?style=for-the-badge&labelColor=0a0a0a" alt="Live Demo" /></a>
  <a href="https://github.com/AnnyeongHae/ai-factcheck-portfolio"><img src="https://img.shields.io/badge/GitHub%20Repo-181717?style=for-the-badge&logo=github" alt="Repo" /></a>
</p>

#### 🖼️ Image RAG

[photoposting.shop](https://photoposting.shop) presents an image and prompt archive. A local owner API uses retrieval/RAG to select references for image generation, with human review kept in the final usage loop.

<p>
  <a href="https://photoposting.shop"><img src="https://img.shields.io/badge/%E2%96%B6%20Live%20Demo-B8F564?style=for-the-badge&labelColor=0a0a0a" alt="Live Demo" /></a>
  <a href="https://github.com/AnnyeongHae/Image_archive"><img src="https://img.shields.io/badge/GitHub%20Repo-181717?style=for-the-badge&logo=github" alt="Repo" /></a>
</p>

#### 🧪 E.F.M — Marketing & Internal Process Automation

- **Blog automation**: a Python workflow connecting keyword extraction, source/news mining, fact checking, LLM drafting, HTML formatting, and output storage
- **SmartStore operations**: Powerlink keyword/ad collection and SERP parsing/classification to support ad efficiency, plus order confirmation, delay messages, and shipping workflows
- **SmartStore detail pages**: a local Python pipeline that turns product images and features into copy, layout, and HTML detail-page outputs
- Marketing and product-positioning experience in an adhesive-materials manufacturing company

#### 🌎 Nammigo — SEO/GEO & Media Asset Automation

- Content optimization for Naver and LLM search across South American travel content
- Python-based photo classification by place and date
- Prompt engineering, image generation, and workflow integration training for marketing teams

### 📚 Published

1. [Mapping Travel Trends Before and After COVID-19 Using Social Network Analysis: Focusing on *Travel Magazine*](https://www.dbpia.co.kr/journal/articleDetail?nodeId=NODE11300884) — Anyong Cheong, Jinyoung Park, Yunjae Nam (2021)
2. [Sentiment Analysis of ‘Non-marital Childbirth’ Using Naver News Comments](https://doi.org/10.5392/JKCA.2022.22.01.074) — Seyoung Heo, Chowon Kim, Anyong Cheong, Saebom Lee (2022)
3. [Analysis of Smart City Media Coverage Using Text Mining and Network Analysis](https://doi.org/10.37727/jkdas.2022.24.4.1563) — Anyong Cheong, Jinyoung Park, Yunjae Nam (2022)
4. [A Comparative Analysis of Korean and U.S. Music Copyright Society Websites Through the Principle of Two-way Symmetrical Communication](https://doi.org/10.9728/dcs.2024.25.7.1931) — Hwayoon Seok, Taehee Park, Anyong Cheong, Yunjae Nam (2024)
5. [A Longitudinal Analysis of VR, AR, and Metaverse Tourism Using Dynamic Topic Modeling of Media Coverage (2016–2023)](https://doi.org/10.17086/JTS.2025.49.5.31.54) — Seongeun Yoon, Anyong Cheong, Yerim Kim, Yunjae Nam (2025)
6. [Pet Owners’ Hotel Choices in South Korea: Key Attributes](https://doi.org/10.1080/1528008X.2026.2639377) — Anyong Cheong, Yunjae Nam (2026)
7. *Hotel Choice Attributes in Pet-friendly Hotel: Text Mining and Conjoint Analysis Approach* — Master’s thesis research

### 🏆 Hackathons & Awards

| Activity | Result / Type | Date | Organizer | Record |
|---|---|---|---|---|
| 현업활용 우수사례 공모전 | 3rd place · Competition | 2025/11/28 | Korea Tourism Organization / Data Marketing Korea | - |
| KB IT’s Your Life Hackathon | Excellence Award · Hackathon | 2024/11/20 → 2024/11/22 | KB Kookmin Bank | [Notion](https://app.notion.com/154e4a6d03ba802295adc8e90ac3ef3f) |
